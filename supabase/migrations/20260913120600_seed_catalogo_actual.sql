...
-- PROTECCIÓN: solo se ejecuta si NO existe ningún producto ni categoría.
-- Si el proyecto ya tiene catálogo cargado, no inserta ni modifica nada.
-- Además usa ON CONFLICT DO NOTHING: nunca sobrescribe precios ni productos.
-- =====================================================================

do $seed$
begin
  if exists (select 1 from public.products) or exists (select 1 from public.categories) then
    raise notice 'Catálogo existente detectado: no se carga el seed ni se modifica nada.';
    return;
  end if;
